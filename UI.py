...
class MY_GUI():
    ...
    #   定义开发窗口大小和形式
        ...
        self.textframe.pack()
        print(Image)
        self.im = Image.open("./4.jpg")
         #括号里为需要显示在图形化界面里的图片
        self.im = self.im.resize((560,50))
        self.img = ImageTk.PhotoImage(self.im)
        Label(self.developer, width=560,height=50,image=self.img).grid(row=0, column=0,columnspan=1)
        Label(self.developer, width=70, height=2, bg="white", text="开发人员：王奕阳\t联系方式：13208016441\t开发时间：2021.4").grid(column=0, row=1)
        # Label(self.developer, width=70, height=2, bg="white", text="当前版本：1.0   ).grid(column=0, row=1)
    ...
    #  定义用户输入局域网内的IP范围和端口窗口，输入命令用来进行扫描
        ...
        self.entry_ip = Entry(self.user_input_label, width=70,bd = 0,textvariable=self.input_ip,background = '#A9A9A9')
        self.entry_ip.insert(0, "请输入需要扫描的IP")
        self.entry_ip.bind("<Button-1>", lambda event: clear_entry(event, self.entry_ip))
        # self.port_label = Label(self.user_input_label, width=70, height=2)
        # self.L2 = Label(self.user_input_label,width=20, text="输入的端口：")
        self.entry_port = Entry(self.user_input_label, width=70,bd = 0,textvariable=self.input_port,background = '#A9A9A9')
        self.entry_port.insert(0, "请输入需要扫描的端口")
        self.entry_port.bind("<Button-1>", lambda event: clear_entry(event, self.entry_port))
        ...
        self.checkftpbox_input_label = LabelFrame(self.checkftpbox_input, text="勾选需要扫描的FTP相关内容")
        self.checkftpbox_input_label.pack(padx=10,pady=10)
        self.ftp1_checkbox = Checkbutton(self.checkftpbox_input_label, text = "FTP匿名登录漏洞", onvalue = 1,variable = self.ftp1_CheckVar, offvalue = 0, height=1, width = 20)
        self.ftp2_checkbox = Checkbutton(self.checkftpbox_input_label, text = "FTP账户密码漏洞", onvalue = 1,variable = self.ftp2_CheckVar, offvalue = 0, height=1, width = 20)
        self.ftp3_checkbox = Checkbutton(self.checkftpbox_input_label, text = "FTP服务信息", onvalue = 1,variable = self.ftp3_CheckVar, offvalue = 0, height=1, width = 20)
        ...
        self.checkmysqlbox_input_label = LabelFrame(self.checkmysqlbox_input, text="勾选需要扫描的MYSQL相关内容")
        self.checkmysqlbox_input_label.pack(padx=10,pady=10)
        self.mysql1_checkbox = Checkbutton(self.checkmysqlbox_input_label, text = "MYSQL空口令漏洞", onvalue = 1,variable = self.mysql1_CheckVar, offvalue = 0, height=1, width = 14)
        self.mysql2_checkbox = Checkbutton(self.checkmysqlbox_input_label, text = "MYSQL账户密码漏洞", onvalue = 1,variable = self.mysql2_CheckVar, offvalue = 0, height=1, width = 16)
        self.mysql3_checkbox = Checkbutton(self.checkmysqlbox_input_label, text = "MYSQL用户信息", onvalue = 1,variable = self.mysql3_CheckVar, offvalue = 0, height=1, width = 12)
        self.mysql4_checkbox = Checkbutton(self.checkmysqlbox_input_label, text = "MYSQL普通信息", onvalue = 1,variable = self.mysql4_CheckVar, offvalue = 0, height=1, width = 14)
        ...
        self.checkbox_input_label = LabelFrame(self.checkbox_input, text="勾选需要扫描的MONGODB相关内容")
        self.checkbox_input_label.pack(padx=10,pady=10)
        self.mongodb_checkbox = Checkbutton(self.checkbox_input_label, text = "MONGODB未授权访问漏洞", onvalue = 1, variable = self.mongodb_CheckVar,offvalue = 0, height=1, width = 31)
        self.mongodb2_checkbox = Checkbutton(self.checkbox_input_label, text = "MONGODB账户密码漏洞", onvalue = 1, variable = self.mongodb2_CheckVar,offvalue = 0, height=1, width = 32)
        ...
        self.checkbox_input = Frame(self.parent_init_name)
        self.checkbox_input_label = LabelFrame(self.checkbox_input, text="勾选需要扫描的CVE相关内容或者自定义脚本上传的相关内容")
        self.checkbox_input_label.pack(padx=10,pady=10)
        self.cve_checkbox = Checkbutton(self.checkbox_input_label, text = "主机所含的CVE漏洞相关信息", onvalue = 1,variable = self.cve_CheckVar, offvalue = 0, height=1, width = 31)
        self.diy_checkbox = Checkbutton(self.checkbox_input_label, text = "上传自定义脚本所得相关信息", onvalue = 1,variable = self.diy_CheckVar, offvalue = 0, height=1, width = 32)
    ...
    #  查看扫描出来的局域网内主机ip详细信息
        ...
        self.listip = Listbox(self.iplistframe,height=2,width=68,bd = 0, yscrollcommand=self.ip_sb.set,xscrollcommand=self.ip_sb2.set)
    ...
    #  存在的安全漏洞
        ...
        self.listinformation = Listbox(self.ipinformation, height=3,width=68, bd=0, yscrollcommand=self.info_sb.set,xscrollcommand=self.info_sb2.set)
    ...
    #   cve列表界面
        ...
        self.listCveinformation = Listbox(self.cveinformation,height=3, width=68, bd=0, yscrollcommand=self.cve_sb.set,xscrollcommand=self.cve_sb2.set)
    ...
    #  UI界面按钮
        ...
        Button(self.buttonframe,text="开始", command=self.start_scan).grid(column=0, row=0)
        ...
        Button(self.buttonframe, text="上传自定义文件", command=self.upload_file).grid(column=2, row=0)
    ...
    def start_scan(self):
       ...
       # print(self.Listip)
        ...
        if len(ip_alive_list) == 0:
            ...
            #  多线程对每一个ip的详细进行扫描
            a = all_infromation(startport, endport, ip_alive_list, self.get_vars(), self.filename)
            a.Threads()
            
            self.have_cve_bug, self.have_ftp_bug, self.have_mysql_bug, self.have_mongodb_bug = a.get_bug_status()
    ...
    def help(self):
        ...
        textframe.pack()
        Label(developer, width=50, height=2, bg="white", text="环境配置：python2，python-nmap\n").grid(column=0, row=0)
    ...
    def get_vars(self):
        return [self.cve_CheckVar.get(), self.ftp1_CheckVar.get(), self.ftp2_CheckVar.get(), self.ftp3_CheckVar.get(), self.mysql1_CheckVar.get(), self.mysql2_CheckVar.get(),self.mysql3_CheckVar.get(),self.mysql4_CheckVar.get(),self.mongodb_CheckVar.get(),self.mongodb2_CheckVar.get(),self.diy_CheckVar.get()]
    ...
    def advice(self):
        ...
        if self.scanned == 1:
            ...
            if  self.ftp2_CheckVar.get() == 1 and self.have_ftp_bug == 1:
                ...
                text.pack()
                text.insert(INSERT,"FTP漏洞加固：\n")
                text.insert(END,"   自我检查FTP中的账户密码信息，创建管理员账号进行管理，避免账户密码是简单的账户密码，甚至是空密码\n")
            ...
            if  self.mysql2_CheckVar.get() == 1 and self.have_mysql_bug == 1:
                ...
                text2.pack()
                text2.insert(INSERT,"MYSQL漏洞加固：\n")
                text2.insert(END,"  自我检查MYSQL数据库中的账户密码信息，创建管理员账号进行管理，避免账户密码是简单的账户密码，甚至是空密码\n")
            ...
            if  self.mongodb2_CheckVar.get() == 1 and self.have_mongodb_bug == 1:
                ...
                text3.pack()
                text3.insert(INSERT,"MONGODB漏洞加固：\n")
                text3.insert(END,"  自我检查MONGODB数据库中的账户密码信息，创建管理员账号进行管理，避免账户密码是简单的账户密码，甚至是空密码\n")
    ...
    def report(self):
        ...
        input_title = Label(developer, width=70, height=2, bg="white", text="请输入需要保存的名字")
        entry_name = Entry(developer, width=70,bd = 0,textvariable=input_name,background = '#A9A9A9')
        input_title.pack()
        entry_name.pack()
    ...
    def start_process_bar(self):
        ...
        self.bar = ttk.Progressbar(self.process, length=200, mode='indeterminate', orient=HORIZONTAL)
