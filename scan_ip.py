...
def getip(ipaddr):  # 获取局域网扫描主机的列表
    ...
    if (
        ...
        or re.match(
            r"^(([01]{0,1}\d{0,1}\d|2[0-4]\d|25[0-5])\.){3}([01]{0,1}\d{0,1}\d|2[0-4]\d|25[0-5])-(([01]{0,1}\d{0,1}\d|2[0-4]\d|25[0-5])\.){3}([01]{0,1}\d{0,1}\d|2[0-4]\d|25[0-5])$",
            ipaddr,
        )
        or re.match(
            r"^(([01]{0,1}\d{0,1}\d|2[0-4]\d|25[0-5])\.){3}([01]{0,1}\d{0,1}\d|2[0-4]\d|25[0-5])(,(([01]{0,1}\d{0,1}\d|2[0-4]\d|25[0-5])\.){3}([01]{0,1}\d{0,1}\d|2[0-4]\d|25[0-5]))+$",
        ...
        if "-" in ipaddr:
            ...
            ipaddr_3 = (
                startip_split[0] + "." + startip_split[1] + "." + startip_split[2] + "."
            )
...
def getport(port):  # 获取扫描的起始端口和结束端口
    if re.match("^\d{1,5}-\d{1,5}$", port) or re.match("^\d{1,5}$", port):
        if "-" in port:
...
def find_ip_alive(ip):  # ICMP协议可能被屏蔽
    ...
    for line in list(output):
        if not line:
            continue
        if (str(line).upper().find("timeout") >= 0) and (
            str(line).upper().find("TTL") >= 0
        ):
            continue
        if (str(line).upper().find("TTL") < 0) and (
            str(line).upper().find("timeout") >= 0
        ):
            continue
        if (str(line).upper().find("TTL") >= 0) and (
            str(line).upper().find("timeout") < 0
